...
export function StandupGoal(props: StandupGoalProps) {
  ...
  const search = (query: any) => {
    if (settings.goal.search) {
      fetch(
        `${environment.api}goals/search?${query.key}=${query.value}`,
        requestOptions
      )
        .then((res) =>
  ...
  useEffect(() => {
    fetch(`${environment.api}users/me/settings`, requestOptions)
      ...
      .then((data) => {
        if (
          !data.settings.goal ||
          Object.keys(data.settings.goal).length === 0
        ) {
          setShowAlert(true);
          setSettings({
            ...settings,
      ...
      <div>
        <Container>
          <Alert show={showAlert} variant="warning">
            ...
            <p>We've noticed missing settings! You can chose ...</p>
            <ul>
              <li>
                if the latest goal should be taken automatically, do not worry
                you can always reset the form
              </li>
              <li>
                if the goal should be searched after blur the Ticket field
              </li>
            </ul>
            <hr />
              ...
              <Col>
                <Row>
                  <Form.Group as={Col} id="formGridLast">
                    <Form.Check
                      label="Apply last goal automatically"
                      type="checkbox"
                      name="last"
                      checked={settings?.goal?.last || false}
                ...
                <Row>
                  <Form.Group as={Col} id="formGridSearch">
                    <Form.Check
                      label="Apply existing goal from the Ticket field"
                      type="checkbox"
                      name="search"
                      checked={settings?.goal?.search || false}
                      onChange={handleSettingsChange}
                    />
                  </Form.Group>
                </Row>
          ...
          <Form onSubmit={onGoalSubmit}>
            ...
            <Row className="mb-3">
              <Form.Group as={Col} id="formGridBlocked">
                <Form.Check
                  type="checkbox"
                  checked={goal.blocked}
                  label="Blocked"
                  name="blocked"
                  onChange={handleChange}
                />
                <p className="text-secondary">
                  I am prevented from reaching somewhere by someone or something
                </p>
              </Form.Group>
